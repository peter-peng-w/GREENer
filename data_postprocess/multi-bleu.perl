...
$bleu_1 = $brevity_penalty * exp((my_log( $bleu[1] ))) ;
$bleu_2 = $brevity_penalty * exp((my_log( $bleu[1] ) +
				my_log( $bleu[2] ) ) / 2) ;
$bleu_3 = $brevity_penalty * exp((my_log( $bleu[1] ) +
				my_log( $bleu[2] ) +
				my_log( $bleu[3] ) ) / 3) ;
$bleu_4 = $brevity_penalty * exp((my_log( $bleu[1] ) +
				my_log( $bleu[2] ) +
				my_log( $bleu[3] ) +
				my_log( $bleu[4] ) ) / 4) ;

printf "BLEU-1 = %.2f, BLEU-2 = %.2f, BLEU-3 = %.2f, BLEU-4 = %.2f, unigram/bigram/3gram/4gram: %.1f/%.1f/%.1f/%.1f (BP=%.3f, ratio=%.3f, hyp_len=%d, ref_len=%d)\n",
    100*$bleu_1,
    100*$bleu_2,
    100*$bleu_3,
    100*$bleu_4,
